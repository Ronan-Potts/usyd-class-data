...
This report analyses data from a survey addressed to students at the University of Sydney (@fig-usyd-quad ) who take *Data Analytics: Learning from Data* (DATA2x02). The survey was run from 15 August 2022 to 23 August 2022 and contains 38 questions which can be found at [this University of Sydney GitHub page](https://pages.github.sydney.edu.au/DATA2002/2022/extra/DATA2x02_survey_2022.pdf). The data can be downloaded [here](https://docs.google.com/spreadsheets/d/e/2PACX-1vTML6kYv2rDjKbPTBuSoX6bdkAWb5Hcm7IWxFmpqftgpt-AFiRhSdDn68XKjGAC_SqQHspAw78tMMuQ/pub?gid=1782171849&single=true&output=tsv). Participants found the survey on the DATA2x02 edstem page, and the survey was advertised on the DATA2x02 edstem page as well as in the DATA2x02 lectures.
...
The calculations in this report were performed in R [@R-base] within the Quarto environment [@quarto] for reproducibility. Data wrangling was performed in base R and the tidyverse [@tidyverse2019] with a single use of naniar [@naniar]. Graphs were produced with ggplot2 [@tidyverse2019]. Each code chunk in this report features `package_name::` before every non-base and non-ggplot2 function that is used.
...
name_combo |>
  dplyr::filter(name_combo$New %in% important_vars) |>
  gt::gt()
...
The data in both cohorts is, however, reasonably symmetric. This can be seen in the density histograms in @fig-histwam .
...
df |>
  ...
  geom_vline(data=avg_2x02, aes(xintercept=avg, color=normal_advanced),
             linetype="dashed") + 
  facet_grid(cols = vars(normal_advanced)) + 
  labs(x="Weighted Average Mark (WAM)", y="Proportion in D2x02 Group", fill="Stream") +
  guides(color=FALSE)

```

We are saved by the Central Limit Theorem here, as there is a large enough sample size of 33 DATA2902 students and 120 DATA2002 students. Therefore, I use a two-sample t-test test

Lets first look at the mean, standard deviation, and sample size of each group (@tbl-wamsummary ).

```{r}
#| label: tbl-wamsummary
#| tbl-cap: "Summary statistics for WAM across the two DATA2x02 streams."
summary_df = df |>
  dplyr::group_by(normal_advanced) |>
  dplyr::summarise(Mean = mean(wam), SD = sd(wam), n=n())
...
t_test = stats::t.test(d2902_wam, d2002_wam, alternative="greater", paired=FALSE, var.equal=FALSE, correct=FALSE, conf.level = 0.95)
```

1.  **Hypothesis:** Let $\mu_\text{Adv}$ be the mean WAM of *DATA2902* students, and $\mu_\text{Nml}$ be the mean WAM of *DATA2002* students. The null hypothesis is $H_0: \mu_\text{Adv} = \mu_\text{Nml}$ and the alternative hypothesis $H_1: \mu_\text{Adv} > \mu_\text{Nml}$.
2.  **Assumptions:** the two population means follow a normal distribution and are roughly symmetric (@fig-histwam ). The populations are identically distributed and sampling is performed independently.
3.  **Test statistic:** $t_0 = \frac{\mu_\text{Adv} - \mu_\text{Nml}}{\sqrt{\frac{\sigma^2_\text{Adv}}{n_\text{Adv}}+\frac{\sigma^2_\text{Nml}}{n_\text{Nml}}}}$ where $\mu_\text{Adv}=$ `r signif(mean(d2902_wam), 3)`, \$\\mu\_\\text{Nml} = \$ `r signif(mean(d2002_wam), 3)`, $\sigma_\text{Adv} =$ `r signif(sd(d2902_wam), 3)`, $\sigma_\text{Nml} =$ `r signif(sd(d2002_wam), 3)`, $n_\text{Adv} =$ `r length(d2902_wam)`, and $n_\text{Nml} =$ `r length(d2002_wam)`.
4.  **Observed test statistic:** $t_0=$ `r signif(as.numeric(t_test["statistic"]), 3)`
5.  **p-value:** $P(t_{151}\leq$ `r signif(as.numeric(t_test["statistic"]), 3)` $) < 0.0001$
6.  **Decision:** Since the p-value is less than 0.05, we reject the null hypothesis in favour of the alternative hypothesis. Hence, we conclude that *DATA2902* students achieve a higher WAM than *DATA2002* students, on average.
...
We can attempt to visualize a correlation by looking at the data among those who do work, as in @fig-histscat .
...
The visualizations show that generally students with higher WAMs tend to work fewer hours, with a more negative correlation among DATA2902 students. We also see that the distribution of Employment Hours among each stream is identical but neither normal nor symmetric. We will use a Wilcoxon rank-sum test here.

```{r}
#| output: false
x = df[df$normal_advanced == "DATA2902",]$employment_hrs
y = df[df$normal_advanced == "DATA2002",]$employment_hrs
r = rank(c(x, y))
w_sum = c("W" = sum(r[1:length(x)]))
min_w = length(x)*(length(x)+1)/2

p_val = 2*stats::pwilcox(w_sum - min_w - 1, length(x), length(y), lower.tail=FALSE)
```

1.  **Hypothesis:** Let $\mu_\text{Adv}$ be the mean hours worked for *DATA2902* students, and $\mu_\text{Nml}$ be the mean hours worked for *DATA2002* students. The null hypothesis is $H_0: \mu_\text{Adv} = \mu_\text{Nml}$ and the alternative hypothesis $H_1: \mu_\text{Adv} \neq \mu_\text{Nml}$.
2.  **Assumptions:** the two populations are independent and follow the same distribution but differ by a shift (@fig-histscat ).
3.  **Test statistic:** $W = R_1 + R_2 +\ ...\ + R_{33}$ where \$R_i\\in \\text{ranks}(X\_\\text{Adv})\$. Under \$H_0\$, $W$ follows the $WRS(n_\text{Adv}, n_\text{Nml})$ distribution.
4.  **Observed test statistic:** $w=$ `r w_sum`
5.  **p-value:** $2P(W\geq$ `r w_sum` $) =$ `r signif(p_val,3)` $< 0.05$
6.  **Decision:** Since the p-value is less than 0.05, we reject the null hypothesis in favour of the alternative hypothesis. Hence, we conclude that *DATA2902* students do not work the same number of hours as *DATA2002* students. Moreover, @tbl-employmentsummary shows that *DATA2902* students work more hours than *DATA2902* hours, on average.

```{r}
#| label: tbl-employmentsummary
#| tbl-cap: "Summary statistics for employment hours across the two DATA2x02 streams."

summary_df = df |>
  dplyr::group_by(normal_advanced) |>
  dplyr::summarise(Mean = mean(employment_hrs), SD = sd(employment_hrs), n=n())
...
The previous result informs us that the success of DATA2902 students could be explained by a greater work ethic. If such a statement is true, does this greater work ethic also come paired with a different sport preference? Could the better results of DATA2902 be explained by a more unique sport lifestyle? While the latter cannot be definitively confirmed by this study, as many other confounding variables may also be involved, we can certainly address the prior.

Before we perform a hypothesis test on the data, we need to investigate some of the data at hand. Hence, we produce a contingency table for each sport among DATA2x02 students (@tbl-sportcontingency ) using some code from @tarr2022 .

```{r}
#| label: tbl-sportcontingency
#| tbl-cap: "Summary statistics for employment hours across the two DATA2x02 streams."
sport <- df |>
...
This contingency table can be extended to a more visual bar chart (@fig-barsport ) for each of the groups of students by using code from @tarr2022 . Visually, it appears that the distributions for the number of participants in each sport is approximately the same in both the *DATA2902* and *DATA2002* class.

```{r, fig.asp=0.5, out.width= "100%", warning=FALSE, message=FALSE}
#| label: fig-barsport
#| fig-cap: "Bar charts of students' sport choices"
sport <- df %>%
  dplyr::select(identifier, sport, normal_advanced) %>% 
  tidyr::separate_rows(sport,
                       sep = ", ") %>% 
...
sport %>%
  ggplot() + 
  aes(y = reorder(sport, sport, function(x) length(x)), fill=normal_advanced) +
  geom_bar() + 
  facet_grid(cols = vars(normal_advanced)) +
  labs(y = "", x = "Count", fill="Stream")
```

As many of the entries in this table contain values that are less than five, I will use a Monte Carlo permutation test.

```{r, warning=FALSE}
set.seed(2022)
sport_data = as.matrix(cont_table)
chi_test = stats::chisq.test(sport_data, simulate.p.value=TRUE, B=10000)
```

1.  **Hypothesis:** $H_0: p_{ij} = p_{\bullet j} = y_{\bullet j}/n$ and $H_1: p_{ij}\neq p_{\bullet j}$.
2.  **Assumptions:** the observations are exchangeable. This is justified via the random assignment in the Monte Carlo test.
3.  **Test statistic:** $t_0 = \displaystyle\sum^2_i\sum^{20}_j\frac{(y_{ij}-e_{ij})^2}{e_{ij}} \not\sim \chi^2_{20}$ as some cell counts are less than 5.
4.  **Observed test statistic:** $t_0 =$ `r signif(chi_test$statistic, 3)` .
5.  **p-value:** $P(T \geq t_0) = \frac{n(T\geq t_0)}{n_\text{total}} =$ `r signif(chi_test$p.value, 3)` $\geq 0.05$.
6.  **Decision:** Since the p-value is greater than 0.05, we cannot reject the null hypothesis in favour of the alternative hypothesis. Indeed, @fig-barsport confirms that the proportion of students who participate in each sport across *DATA2902* and *DATA2002* is roughly equal.

# Results

Despite all of the cleaning and analysis above, there is still one elephant in the room; the DATA2x02 students who responded to the survey are not representative of the entire DATA2002 cohort. This opens the conclusions drawn in this report to be influenced by many biases, such as non-response/self-selection bias which inherently induces an under-coverage of the DATA2x02 cohort.

Fortunately, it appears as if these biases have not had a large impact on factors such as WAM. For example, the average WAM across the entire responding cohort is `r signif(mean(df$wam), 2)` which is approximately equal to the current average WAM at the University of Sydney (see [this](https://imgur.com/a/TFZzTZv) semester average mark distribution across the USYD faculty of engineering in 2018).

Moreover, while these biases may have had a net impact over the entire cohort of responders, by comparing and contrasting the summary statistics of two groups of responders, we reduce the potential impact of any bias. Hence, the aim of this report still has integrity in stating that:

1.  DATA2902 students perform better, on average, than DATA2002 students,
2.  DATA2902 students work more, on average, than DATA2002 students,
3.  DATA2902 students and DATA2002 students participate in the same sports in equal proportions.

It is worthwhile to note that while the above is probably true on average across the entire DATA2x02 cohort, it cannot reliably be used on an individual level, even if we were to ignore the ethical boundaries of such an action. For example, suppose that you did choose to compare a random DATA2902 student to a random DATA2002 student via WAM.
...
rand_n_D2902 = sample(unlist(df[df$normal_advanced == "DATA2902", "wam"]), size=n, replace=TRUE)
rand_n_D2002 = sample(unlist(df[df$normal_advanced == "DATA2002", "wam"]), size=n, replace=TRUE)

sum(rand_n_D2902 > rand_n_D2002) / n
```

The percentage of the time that the DATA2902 has a higher WAM is `r sum(rand_n_D2902 > rand_n_D2002) / n` which is by no means statistically significant.
