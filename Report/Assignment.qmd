...
df |>
  ...
  geom_vline(data=avg_2x02, aes(xintercept=avg, color=normal_advanced),
             linetype="dashed") + 
  facet_grid(cols = vars(normal_advanced), labeller = ) + 
...
3.  **Test statistic:** $t_0 = \frac{\mu_\text{Adv} - \mu_\text{Nml}}{\sqrt{\frac{\sigma^2_\text{Adv}}{n_\text{Adv}}+\frac{\sigma^2_\text{Nml}}{n_\text{Nml}}}}$ where $\mu_\text{Adv}=80.4$, $\mu_\text{Nml} = 72.8$, $\sigma_\text{Adv} =9.3$, $\sigma_\text{Nml} = 10.4$, $n_\text{Adv} = 33$, and $n_\text{Nml} = 120$.
4.  **Observed test statistic:** $t_0=4.03$
5.  **p-value:** $P(t_{151}\leq 4.03) < 0.0001$
...
2.  **Assumptions:** the two populations are independent and follow the same distribution but
    differ by a shift (@fig-histscat ).
...
The previous result informs us that the success of DATA2902 students could be explained by their greater work ethic. Does this greater work ethic also come paired with a more active lifestyle? Could the better results of DATA2902 be explained by such an active lifestyle, confirming the countless studies which suggest that exercise boosts your mental ability? While the latter cannot be confirmed by this study, as many other confounding variables may also be involved, we can certainly address the prior.

Before we perform a hypothesis test on the data, we need to investigate some of the data at hand. Hence, we produce a contingency table for each sport among DATA2x02 students. First modify a code snippet from @tarr2022 below to create a more friendly dataframe.
...
This contingency table can be extended to a more visual bar chart.
