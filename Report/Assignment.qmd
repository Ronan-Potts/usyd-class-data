...
t_test = t.test(d2902_wam, d2002_wam, alternative="greater", paired=FALSE, var.equal=FALSE, correct=FALSE, conf.level = 0.95)
```
...
w_sum
2*pwilcox(w_sum - min_w - 1, length(x), length(y), lower.tail=FALSE)
```
...
5.  **p-value:** $2P(W\geq$ `r w_sum` $) < 0.05$
...
chi_test = chisq.test(sport_data, simulate.p.value=TRUE, B=10000)
```
